...
learn_incidents:
  parent: Detection
canonical: 'https://www.checklyhq.com/docs/learn/incidents/reduce-mttd/'
---
...
- Set retry policies — requests sometimes fail via the internet. Set a reasonable [retry policy](/communicate/alerts/overview) on your checks so you’re not generating false alarms.
...
A tool like [Playwright](/detect/synthetic-monitoring/browser-checks/playwright-support) is the best way to really act like a user when interacting with our service. Checkly uses Playwright to let engineers test every aspect of their service with the same expectations their users bring.
