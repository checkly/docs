...
2. **Resolver selection**: The query is sent to a DNS resolver:
   - **Default**: 8.8.8.8 (Google DNS) with automatic failover. If the default resolver returns an error code (SERVFAIL, NXDOMAIN, etc.), Checkly automatically retries with 1.1.1.1 (Cloudflare DNS), then an internal AWS resolver
   - **Custom resolver**: You can specify a custom nameserver and port for testing resolver-specific behavior (no automatic failover, queries only use your specified nameserver)
