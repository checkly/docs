...
Snippets are JavaScript files which can be reused across [Browser checks](/browser-checks/), as well as [setup and teardown scripts](/api-checks/setup-teardown-scripts/) for API checks. By reusing code with snippets, checks can be made more maintainable and easier to develop.
