...
For a [`merge_group` event](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#merge_group), the Action uses `GITHUB_SHA`, which identifies the temporary merge-group commit. These requirements are GitHub branch-protection concerns; they do not require Checkly's internal CI workflows, SHA propagation, or deployment-provider setup.
