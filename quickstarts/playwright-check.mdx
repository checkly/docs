...
<Tip>
Explore all [Playwright Check Suite capabilities](/detect/synthetic-monitoring/playwright-checks/overview) in the complete overview.
</Tip>
...
- `checkly test` - Runs check suites defined in `checkly.config.ts`
- `checkly pw-test` - Runs any Playwright tests directly on Checkly's cloud infrastructure. See the [CLI reference](/cli/checkly-pw-test).
</Info>
