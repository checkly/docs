...
| `API_CHECK_RESPONSE_STATUS_CODE` | HTTP status code (API checks only) | 404 |
| `API_CHECK_RESPONSE_STATUS_TEXT` | HTTP status text (API checks only) | "Not Found" |
...
| `REGION` | AWS region code where check ran | "us-east-1" |
...
| `SSL_CHECK_DOMAIN` | Domain for SSL certificate check | "api.example.com" |
| `SSL_DAYS_REMAINING` | Days until SSL certificate expires | 14 |
