...
<Note>
Only resources that are not already managed by a CLI project are importable. If the command reports `No importable resources were found`, follow the [import troubleshooting guide](/cli/importing#troubleshoot-no-importable-resources-were-found).
</Note>
