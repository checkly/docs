...
Here are common ways to use Checkly's built-in environment variables in your tests:
...
Different behavior for scheduled runs versus manual triggers:
...
test('geo-specific content test', async ({ page }) => {
  ...
  if (process.env.CHECKLY_REGION?.startsWith('eu-')) {
    // Expect EU-specific content. Cookie banner anyone?
  }
});
```
