...
### 2. [Optional] If you're using TypeScript
  ...
  If you're using TypeScript, install the dev dependencies [`jiti`](https://www.npmjs.com/package/jiti). Jitti will enable the CLI to bundle your typescript config and test files correctly.
...
The `pw-test` command enables you to run your existing tests in the Checkly infrastructure. Use the `--create-check` flag to create a `checkly.config.ts` file if it doesn't exist. The `checkly.config` will include the necessary configuration to run your Playwright tests as synthetic monitors. and will add the new check.
...
<Info>
The difference between `checkly test` and `checkly pw-test` is that `test` takes all your defined monitors and runs them in the Checkly infrastructure. `pw-test` only parses and runs your existing Playwright tests with the provided CLI configuration.
</Info>
...
Lern more about [alerting with Checkly](/communicate/alerts/overview) and [all available alert channels](/communicate/alerts/channels).
