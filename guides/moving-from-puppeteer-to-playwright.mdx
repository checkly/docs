...
canonical: 'https://www.checklyhq.com/docs/guides/moving-from-puppeteer-to-playwright/'
---
...
Checkly users switching to Playwright can take advantage of its new Rich Browser Check Results, which come with [tracing and Web Vitals](/detect/synthetic-monitoring/browser-checks/performance-metrics) and make it easier to isolate the root cause of a failed check and remediate faster.
...
> Aside from running a Playwright script, performance and error tracing also require the use of [Runtime](/platform/runtimes/overview) `2021.06` or newer.

> Note that cross-browser support is not available on Checkly - [our Browser checks run on Chromium](/detect/synthetic-monitoring/browser-checks/overview) only.
