...
| `checkly_icmp_check_timing_seconds` | Histogram | The timing phases of the ICMP check, including DNS resolution and latency measurements (avg, min, max). |
| `checkly_icmp_check_packet_loss_ratio` | Gauge | The average packet loss ratio (0-1) for the ICMP check. |
| `checkly_dns_check_timing_seconds` | Histogram | The total duration of the DNS check. |
...
The metrics `checkly_browser_check_web_vitals_seconds`, `checkly_browser_check_errors`, `checkly_api_check_timing_seconds`, `checkly_icmp_check_timing_seconds`, and `checkly_dns_check_timing_seconds` contain a `type` label.
This label indicates the different Web Vitals, error types, and timing phases being measured.

For `checkly_icmp_check_timing_seconds`, the `type` label has the following values: `dns` (DNS resolution time), `latency_avg`, `latency_min`, and `latency_max`.
For `checkly_dns_check_timing_seconds`, the `type` label has the value `total`.
...
| `check_type` | The type of check: `API`, `BROWSER`, `MULTI_STEP`, `HEARTBEAT`, `TCP`, `ICMP`, `DNS`, etc. |
