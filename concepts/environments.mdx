...
See [Turn your Playwright tests into monitors](/guides/playwright-testing-to-monitoring#step-1-pick-the-tests-that-become-monitors) for switching `baseURL` between local and Checkly runs, and the [built-in environment variables](/detect/synthetic-monitoring/playwright-checks/environment-variables) for finer control.
