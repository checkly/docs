...
new GrpcMonitor('grpc-behavior-1', {
  ...
  request: {
    ...
    assertions: [
      ...
      GrpcAssertionBuilder.responseMessage('$.name').isNotEmpty(),
    ],
  },
})
```
...
- `healthCheckStatus()`: Assert the health status string (HEALTH mode): `SERVING`, `NOT_SERVING`, `UNKNOWN`, `SERVICE_UNKNOWN`
...
- Assert the health check reports SERVING:

```ts
GrpcAssertionBuilder.healthCheckStatus().equals('SERVING')
// Equivalent to:
{ source: 'GRPC_HEALTHCHECK_STATUS', comparison: 'EQUALS', target: 'SERVING' }
```

- Assert a specific field in the JSON response body:

```ts
GrpcAssertionBuilder.responseMessage('$.userId').isNotEmpty()
// Equivalent to:
...
message Order { string order_id = 1; string status = 2; }`,
    ...
    assertions: [
      GrpcAssertionBuilder.statusCode().equals(0),
      GrpcAssertionBuilder.responseMessage('$.status').isNotEmpty(),
    ],
  },
})
```
