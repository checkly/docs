...
tags:
  - AI
canonical: 'https://www.checklyhq.com/docs/guides/claude-code-monitoring/'
---
...
Deploy preview · Danube Web Shop → account Monitoring as Code

  + BrowserCheck  __checks__/crime-category.spec.ts        __checks__/crime-category.spec.ts
  + BrowserCheck  __checks__/fantasy-category.spec.ts      __checks__/fantasy-category.spec.ts
  + BrowserCheck  __checks__/homepage-elements.spec.ts     __checks__/homepage-elements.spec.ts
  + BrowserCheck  __checks__/navigation-menu.spec.ts       __checks__/navigation-menu.spec.ts
  + BrowserCheck  __checks__/search-functionality.spec.ts  __checks__/search-functionality.spec.ts
  + BrowserCheck  book-detail-visibility-check-v1          __checks__/book-detail.check.ts
  + BrowserCheck  signup-form-elements-check-v1            __checks__/signup-form.check.ts
  - Check         danube-web-shop-monitor                  permanently deleted, run history lost
    1 unchanged

7 to create, 1 to delete, 1 unchanged
...
* Checks with a matching logical ID from a previous deployment are marked `~`, with a diff of what changed below the overview, or only counted in the `N unchanged` row when nothing did.
