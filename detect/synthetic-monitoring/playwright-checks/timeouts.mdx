...
3. **Dependency installation**: If the dependencies cache doesn't exist, Checkly runs the install command (default: `npm install`, `pnpm install` or `yarn install`).
   ...
   - Customize the install command using `installCommand` in your [check suite configuration](/detect/synthetic-monitoring/playwright-checks/configuration#customize-install-and-test-commands)
...
- The timeouts of these stage are set through the timeouts in your Playwright tests and configuration.
...
```
Error: Dependency installation exceeded 5 minute timeout
```
...
```
Error: Check suite exceeded 30 minute global timeout
```
