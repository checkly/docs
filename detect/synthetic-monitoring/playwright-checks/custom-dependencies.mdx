...
<Info>
**Runtime Environment**: Playwright Check Suites run on Node.js 22 with a minimum of 3.3 CPU cores and 6.8 GiB memory. See the [runtime specification](/platform/runtimes/runtime-specification) for further details.
</Info>
...
Checkly caches installed dependencies between check runs to speed up execution and reduce installation time. This means your dependencies are installed once and reused for subsequent runs until your `package.json` and lock files change.

### How caching works

When dependency caching is enabled:

1. On the first run, Checkly installs all dependencies from your `package.json` and stores them in cache
2. On subsequent runs, Checkly checks if your `package.json` and lock files have changed
3. If unchanged, Checkly uses the cached dependencies, skipping the installation step
4. If changed, Checkly installs dependencies again and updates the cache
...
**Cloud Locations**: Dependency caching is **Always ON**.
