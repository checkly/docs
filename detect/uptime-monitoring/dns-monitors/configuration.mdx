...
* **DNS server & Port:** Optionally override the default resolver with a specific nameserver (e.g. dns.google). Can be a domain name or an IP address
* **Protocol:** DNS uses UDP by default and upgrades to TCP automatically if required. You can pin the protocol to UDP-only or TCP-only if required
...
```
NOERROR 0ms
;; opcode: QUERY, status: NOERROR, id: 59493
...
;; ANSWER SECTION:
checklyhq.com.	27	IN	A	18.66.102.85
checklyhq.com.	27	IN	A	18.66.102.10
checklyhq.com.	27	IN	A	18.66.102.76
checklyhq.com.	27	IN	A	18.66.102.127
```

With JSON path assertions, you could:

* `$.Answer.length` → assert that checklyhq.com always returns exactly 4 A records.

* `$.Answer[0].TTL` → assert that the TTL for a record is above or below a given threshold.
