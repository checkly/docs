...
<Tab title="CLI">
Press <kbd>Ctrl</kbd>+<kbd>C</kbd> during a recorded test session run — `checkly pw-test` (records by default), `checkly test --record`, or `checkly trigger --record` — to open a cancellation prompt.
