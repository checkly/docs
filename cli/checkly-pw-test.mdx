...
| `--detach, -d` | - | Keep checks running in the cloud when you stop the CLI with Ctrl+C; skips the cancellation prompt. |
...
Skip the cancellation prompt. By default, pressing <kbd>Ctrl</kbd>+<kbd>C</kbd> during a `pw-test` run opens a prompt to cancel the recorded test session. With `--detach`, the CLI exits immediately instead, and the checks keep running in the cloud.
...
Cancellation is available only for recorded test sessions. `pw-test` records by default, so its runs can be cancelled unless you pass `--no-record`.

Press <kbd>Ctrl</kbd>+<kbd>C</kbd> during a `checkly pw-test` run to open a cancellation prompt. Pressing <kbd>Ctrl</kbd>+<kbd>C</kbd> a second time exits the CLI without waiting. Pass the `--detach` flag to skip the prompt entirely — <kbd>Ctrl</kbd>+<kbd>C</kbd> then exits immediately and the checks keep running in the cloud.
