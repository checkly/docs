...
But what do you need to consider when you start globally monitoring in production? How do you make your Playwright tests geo location aware? How can you avoid that your tests are counted as real users? And should every test become a Playwright monitor?
...
export function getRetries(env: Environment) {
  ...
  // increase retries to avoid unnessecary monitoring alerts
  const isProd = env === "PROD";
  if (isProd) return 2;
...
If you start running Playwright against production sites, you must ensure to not mess with possible client-side analytics. Luckily, [request interceptions is quickly done in Playwright](/learn/playwright/intercept-requests).

```ts
page.route("**/*", (route) => {
  const blockedDomain = ["analytics.example.com", "tracking.example.com"];
