...
<Note>
Playwright check suites are never part of an import plan, whether you import everything or [name a single check by id](#importing-individual-resources): a suite is defined by the code bundle the CLI uploaded on deploy, and an import cannot turn that bundle back into source files. Naming one reports that no importable resources were found. To manage a suite from a new project, deploy it again from the Playwright project that holds its source.
</Note>
