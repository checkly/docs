...
The three steps below take you from one check in one region to monitoring that sees every market, names the region that broke, and pages only when enough regions agree. Each step feeds the next: the measurement tells you which locations matter, and the locations you choose are what the alert votes with.
...
A URL monitor is cheap enough to run from all of them, which makes it the right instrument for measuring.
