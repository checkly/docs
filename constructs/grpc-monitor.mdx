...
canonical: 'https://www.checklyhq.com/docs/constructs/grpc-monitor/'
---
...
- In BEHAVIOR mode: the fully-qualified method name and a service definition. Use server reflection, an inline `.proto` file, or a compiled FlatBuffers `.bfbs` schema.
- For FlatBuffers: install the [`flatc` compiler](https://flatbuffers.dev/flatbuffers_guide_using_schema_compiler.html)
...
The command creates `schema.bfbs`. The `--bfbs-builtins` flag preserves attributes used for streaming and nested FlatBuffers fields. Load that binary file as base64 in your monitor:

```ts grpc-flatbuffers.check.ts
import { readFileSync } from 'node:fs'
import { GrpcAssertionBuilder, GrpcMonitor } from 'checkly/constructs'

new GrpcMonitor('grpc-flatbuffers', {
  name: 'FlatBuffers gRPC service',
  request: {
    url: 'grpc.example.com',
    port: 443,
    grpcConfig: {
...
<Note>
Do not set `serviceDefinition` or `protoContent` for FlatBuffers. The compiled `bfbsContent` provides the service definition.
</Note>

Recompile the `.fbs` source whenever it changes. Checkly uploads the compiled `.bfbs` content and does not compile the source schema during a test or deployment.
...
| `encoding` | `string` | ❌ | `'PROTOBUF'` | Request and response encoding: `'PROTOBUF'` \| `'FLATBUFFERS'`. BEHAVIOR mode only. |
...
| `bfbsContent` | `string` | FLATBUFFERS | - | Base64-encoded compiled `.bfbs` schema. Required when `encoding` is `'FLATBUFFERS'`. |
