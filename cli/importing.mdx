...
canonical: 'https://www.checklyhq.com/docs/cli/importing/'
---
...
### Troubleshoot "No importable resources were found"

Only resources that are not already managed by a Checkly CLI project can be imported. Specifying a resource ID does not override its project association.

If no resources are importable, check whether the resource already belongs to this or another CLI project. A resource in the current project is already managed and does not need to be imported. Also check for an applied but uncommitted import: finish it with `npx checkly import commit`, or cancel it with `npx checkly import cancel` before trying again.

The `Found ... exported resources` message counts constructs exported by your local project. It does not count account resources available to import.
