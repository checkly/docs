...
- `jsonAnswer(property?)`: The DNS response in JSON format. This allows you to target specific fields using JSON path assertions. The response structure varies by record type. [Learn more about using JSON path](/detect/synthetic-monitoring/api-checks/assertions#json-responses-with-json-path).
