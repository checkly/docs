...
## How does an ideal Playwright Check Suite look like?

While a Playwright Check Suite can reflect and run your entire existing end-to-end test suite, **we recommend splitting your Playwright test suite into multiple Playwright Check Suites to fulfill your monitoring requirements**.

Playwright tests are run to validate a pre-production environment and prepare a production release. Full end-to-end test executions going way beyond fifteen minutes are common for this use case. And while running all the tests in preparation makes sense for testing, it is not ideal for monitoring.

**The longer your Playwright Check Suite runs, the longer you have to wait until you will receive an alert in case of a failure.** Your Playwright Check Suites should execute and fail quickly to guarantee a short Mean Time to Detect (MTTD).
...
<Tip>
What test duration is acceptable depends on your application and how quickly you want to be alerted in case of an issue. If you're service level agreements (SLAs) require a fast reaction time, we recommend to keep your Playwright Check Suites as short as possible.

Remember: 99.9% uptime allows for 1m 26s downtime a day. To guarantee a high-level of uptime, your Playwright Check Suites should run significantly faster than that.
</Tip>
