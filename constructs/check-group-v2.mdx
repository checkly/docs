...
An [AlertEscalationPolicy](/constructs/alert-escalation-policy) object defines [alert-settings](/docs/communicate/alerts/overview) for Check runs. 
...
To prevent issues, Checkly validates group settings against the lowest common denominator of supported features across all monitors in the group.

See [Mixing Checks and Monitors in a Group](/platform/groups/#mixing-checks-and-monitors-in-a-group) for details and examples.
