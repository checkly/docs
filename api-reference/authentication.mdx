...
<Card title="Quick API Testing" icon="flask">
Test your API credentials and connection quickly
```bash
curl -I https://api.checklyhq.com/v1/checks
```
</Card>

<Card title="SDK Configuration" icon="code">
Set up authentication for JavaScript/Python SDKs
```javascript
const client = new ChecklyAPI(apiKey, accountId)
```
</Card>

<Card title="CI/CD Integration" icon="cogs">
Authenticate in automated workflows and deployments
```bash
export CHECKLY_API_KEY=${{ secrets.CHECKLY_API_KEY }}
```
</Card>

<Card title="Secure Environment Setup" icon="shield">
Manage credentials securely across environments
```bash
echo "CHECKLY_API_KEY=cu_xxx" >> .env.production
```
</Card>
...
<Accordion title="Account ID Location">
...
<Card title="Missing Authentication" icon="exclamation-triangle">
...
<Card title="Missing Account ID" icon="building">
