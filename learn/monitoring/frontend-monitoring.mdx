...
---
**Ready to start frontend monitoring today?** check out our practical guide on [how to start frontend monitoring with Checkly and Playwright.](/guides/monitoring-ecommerce-apps-using-playwright)
...
- **API Response Times**: Speed and reliability of API calls. This may be called ‘[heartbeat monitoring](/detect/uptime-monitoring/heartbeat-monitors/overview)’ if you’re only measuring the reliability of straightforward `get` requests.
...
- **Monitoring Page Load Times**: [Ensure optimal page rendering speed](/detect/synthetic-monitoring/browser-checks/performance-metrics#total-blocking-time).
