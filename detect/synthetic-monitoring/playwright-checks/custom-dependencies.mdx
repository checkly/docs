...
By default, Checkly installs all dependencies (both `dependencies` and `devDependencies`). You can change this account-wide setting to install only `devDependencies` in your [account settings](https://app.checklyhq.com/accounts/settings/account/playwright).
...
To override the account-wide setting for a specific check suite, use the `installCommand` property in your `checkly.config.ts`:
