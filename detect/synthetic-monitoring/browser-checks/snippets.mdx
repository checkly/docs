...
<Warning>
**Snippets are not supported in CLI-managed checks.** If you manage your checks with the [Checkly CLI](/cli/overview), any `require('./snippets/...')` reference will fail with a `DependencyParseError` at bundle time — snippets only exist in the Checkly platform, not in your local file system.

Instead, use standard `import` / `require` against local files in your repo. See [local dependencies](/cli/dependencies#local-dependencies) for details.
</Warning>
