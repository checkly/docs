...
### Example command customizations

**When to override `installCommand`:**
...
// Skip installing dependencies (if bundled with your code)
installCommand: 'echo "Dependencies already installed"'
```

**When to override `testCommand`:**

```typescript
// Enable Playwright tracing for debugging failures
...
<Note>
The `testCommand` automatically includes your `pwProjects`, `pwTags`, and Playwright config file path so you don't add these flags manually.
</Note>
