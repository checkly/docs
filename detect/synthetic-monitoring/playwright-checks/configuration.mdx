...
canonical: 'https://www.checklyhq.com/docs/detect/synthetic-monitoring/playwright-checks/configuration/'
---
...
To disable automatic trace header injection, define `use.extraHTTPHeaders` in your Playwright configuration. A top-level value disables injection for the whole suite. A project-level value disables it only for that project. An empty object also disables injection. Checkly preserves the headers that you define, and the [built-in trace environment variables](/detect/synthetic-monitoring/playwright-checks/environment-variables#built-in-variables-in-playwright-check-suites) remain available.
