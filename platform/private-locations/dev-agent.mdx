...
The Checkly Agent runs inside your infrastructure as part of Private Locations. Some npm packages include native code that must be compiled during installation. If your checks rely on npm packages that include native code, those packages need to be compiled during installation. The standard agent is optimized for size and doesn’t ship with build tools, so in those cases you’ll need to use the **dev** image instead.
