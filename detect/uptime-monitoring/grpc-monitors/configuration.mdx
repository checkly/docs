...
* **Encoding and service definition**: How the request is encoded and the method schema is resolved:
  ...
  * **FlatBuffers**: Provide a compiled binary FlatBuffers schema (`.bfbs`). Compile the `.fbs` source locally with `flatc -b --schema schema.fbs`; Checkly consumes the generated `.bfbs` file, not the `.fbs` source.
