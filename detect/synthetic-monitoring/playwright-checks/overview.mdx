...
Your standard Playwright end-to-end test suite consists of projects, `spec.ts` files and individual tests. Use Playwright Check Suites to select and regroup the tests you want to run as synthetic monitors.

<Frame>
  ![Playwright Check Suites test selection](/images/playwright-test-selection.jpg)
</Frame>

**A Playwright **Check** Suite is a collection of tests that are defined in one or more `spect.ts` files.** To control which tests become part of your Playrwight Check Suite use:
