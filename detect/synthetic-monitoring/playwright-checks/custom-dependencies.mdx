...
Checkly supports npm and pnpm workspaces in Playwright Check Suites. Yarn workspaces are not supported.

If your Playwright tests are in a monorepo, Checkly automatically detects and installs dependencies for your workspace structure. Packages in your workspace can depend on other packages in the same workspace, including from the root package.
...
In most synthetic monitoring scenarios, these scripts are not needed, since they typically handle tasks like compiling native bindings, generating code, or setting up Git hooks, none of which apply at check runtime.
