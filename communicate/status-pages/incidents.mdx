...
6. Compose the initial incident update message, and adjust the status of the incident.
...
Your incident will now appear on all status pages that show the selected service(s). If you ticked the option to notify subscribers, they will receive the update message by email.
...
Incident automation allows to automatically open incidents whenever a check fails. It works by connecting a check to a service. When the check fails and triggers an alert it will also open an incident on the connected service. Multiple checks can be connected to the same service through incident automation. The incident will be automatically resolved when the check recovers.
