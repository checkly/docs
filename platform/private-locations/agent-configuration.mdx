...
The agent container runs two processes that each make outbound TLS connections to Checkly: the main agent process and a Node.js worker. Each uses its own trust store, so you typically need to configure both.

First, add your Certificate Authority's (CA) root certificate to the system's CA store. On Debian, and related Linux distributions, you can do this by copying your `*.crt` certificate to `/usr/local/share/ca-certificates/` and running `sudo update-ca-certificates`. This is required for the main agent process.

Then, configure the Node.js worker to trust the certificate. Copy the certificate as a PEM file onto the host, then update the Docker run command to mount the certificate as a volume and pass the path to the certificate in the `NODE_EXTRA_CA_CERTS` environment variable. For a certificate stored at the path `~/certificate.pem`, the Docker run command will be:
...
Alternatively, you can tell the Node.js worker to accept unauthorized certificates with the environment variable `NODE_TLS_REJECT_UNAUTHORIZED=0`. This can be appended to your `docker run ..` command with the `-e` flag.
