...
learn_playwright:
  parent: "Basics"
canonical: 'https://www.checklyhq.com/docs/learn/playwright/mock-api/'
---
...
As we find ourselves testing correct rendering of a page under unusual conditions, this does suggest a [visual regression test](/detect/synthetic-monitoring/browser-checks/visual-regressions) to make sure a page looks correct. Visual regression tests shouldn’t be a default mode for functionality testing, but they are part of our toolkit and editing dynamic API responses can be an effective way of making sure that a page is always rendering with identical data for visual comparison.
...
To go further, see our full documentation on [visual regression checks in Playwright and Checkly](/detect/synthetic-monitoring/browser-checks/visual-regressions#visual-regression--snapshot-testing).
