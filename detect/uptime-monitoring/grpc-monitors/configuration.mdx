...
* **Encoding and service definition**: How the request is encoded and the method schema is resolved:
  * **Protobuf** (default): Use server reflection or provide the `.proto` file content inline.
  * **FlatBuffers**: Provide a compiled binary FlatBuffers schema (`.bfbs`). Compile the `.fbs` source locally with `flatc -b --schema --bfbs-builtins schema.fbs`; Checkly consumes the generated `.bfbs` file, not the `.fbs` source. The `--bfbs-builtins` flag preserves attributes used for streaming and nested FlatBuffers fields.
...
<Note>
For a Checkly CLI example that compiles and loads a FlatBuffers schema, see the [`GrpcMonitor` construct documentation](/constructs/grpc-monitor#use-a-flatbuffers-schema).
</Note>
