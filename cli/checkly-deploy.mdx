...
- `secret changed: <property>` for a property Checkly stores encrypted. Its values are never printed.
...
Print the plan as a JSON envelope and exit without deploying. The envelope carries every resource the deploy would touch and, for each updated one, the changed properties with their values before and after, together with the plan token. A value longer than 256 characters (a script, a request body) is replaced by an `$omitted` marker carrying its length, a changed secret is marked `secret: true` without values, and the deployed state itself is not included. Useful for scripts and agents that want to inspect a deploy before running it with `--plan-token`.
