...
and a set of [3rd party libraries available in each runtime](/runtimes/specs/). Popular use cases are signing HMAC requests, 
...
environment variables](/api-checks/variables/#managing-variables).
...
[setup script examples](/api-checks/setup-script-examples/) for actual examples of OAuth2, HMAC signing, JWT tokens 
...
To learn more about teardown scripts, [check our teardown script examples](/api-checks/teardown-script-examples/).
...
[The setup and teardown runtime](/runtimes/) also exposes a set of specific environment variables to the setup and teardown scripts,
...
Check out [our runtimes documentation](/runtimes/specs/) for a full specification of which modules are included.
...
2. Using [snippets](/snippets/) to DRY (Don't Repeat Yourself) up your code, and referencing those snippets by name.
...
Find more information in the [snippet documentation](/snippets/).

### Using environment variables via Handlebars

If your API Check's request body relies on data evaluated in a setup script, use [environment variables](/api-checks/variables/) to make it accessible.
...
To learn more about setup scripts, [check our setup script examples](/api-checks/setup-script-examples/).
