...
2. [Load timings](/browser-checks/tracing-web-vitals/#first-contentful-paint) for the first page navigation
...
4. An [interactivity](/browser-checks/tracing-web-vitals/#total-blocking-time) summary
![check overview interactivity graph](/images/docs/images/monitoring/check-overview-interactivity.png)

5. A [visual stability](/browser-checks/tracing-web-vitals/#cumulative-layout-shift) breakdown
...
2. When using Playwright Test Runner you will have an additional section displaying the test steps, error message, and assets (traces, videos, screenshots). [Read more](/browser-checks/playwright-test/) about the additonal functionalities of Playwright Test.
![check results browser playwright test report](/images/docs/images/monitoring/check-results-browser-pwt-report.png)
...
The source shows where your ping originated, determined by the `origin` and `referer` request headers. [Learn more about configuring the ping source](/heartbeat-monitors/#pinging-your-heartbeat-monitor).
...
<video
  ...
  src="/images/docs/images/monitoring/check-retries.gif"
></video>

## TCP monitor results

On the [monitor results overview page](/images/docs/monitoring/check-results/#check-results-overview), select a specific check run to review its results:
