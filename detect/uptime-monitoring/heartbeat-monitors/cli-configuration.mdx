---
title: 'CLI Configuration'
description: 'Learn how to configure heartbeat monitors with the Checkly CLI.'
sidebarTitle: CLI
canonical: 'https://www.checklyhq.com/docs/detect/uptime-monitoring/heartbeat-monitors/cli-configuration/'
---


## Overview

The Checkly CLI is a powerful tool for managing your heartbeat monitors. It allows you to define your heartbeat monitors as code alongside your other monitoring, providing version control, code review, and consistent deployment across environments.



## With the Checkly CLI

Define heartbeat monitors as code alongside your other monitoring using the Checkly CLI. This approach provides version control, code review, and consistent deployment across environments.

### TypeScript Example

```typescript
import { HeartbeatMonitor } from 'checkly/constructs'

new HeartbeatMonitor('weekly-newsletter', {
  name: 'Weekly Newsletter Campaign',
  period: 7,
  periodUnit: 'days',
  grace: 4,
  graceUnit: 'hours',
  tags: ['newsletter', 'marketing', 'weekly']
})

new HeartbeatMonitor('database-backup', {
  name: 'Nightly Database Backup',
  period: 1,
  periodUnit: 'days', 
  grace: 30,
  graceUnit: 'minutes',
  tags: ['backup', 'database', 'critical']
})
```

### JavaScript Example

```javascript
const { HeartbeatMonitor } = require('checkly/constructs')

new HeartbeatMonitor('data-sync', {
  name: 'Hourly Data Sync',
  period: 1,
  periodUnit: 'hours',
  grace: 10,
  graceUnit: 'minutes',
  tags: ['sync', 'data', 'hourly']
})
```

### Advanced CLI Configuration

```typescript
import { HeartbeatMonitor, EmailAlertChannel, SlackAlertChannel } from 'checkly/constructs'

const email = new EmailAlertChannel('email', {
  address: 'ops@example.com'
})
const slackCritical = new SlackAlertChannel('slack-critical', {
  url: 'https://hooks.slack.com/services/YOUR/WEBHOOK/URL',
  channel: '#critical'
})

new HeartbeatMonitor('production-backup', {
  name: 'Production Database Backup',
  period: 24,
  periodUnit: 'hours',
  grace: 60,
  graceUnit: 'minutes',
  tags: ['production', 'backup', 'critical'],
  alertChannels: [email, slackCritical],
  activated: true,
  muted: false
})
```

