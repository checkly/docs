...
canonical: 'https://www.checklyhq.com/docs/communicate/maintenance-windows/overview/'
---
...
A clock change can skip over your start time or hit it twice. Either way the window still runs exactly once:

- When clocks jump **forward**, an hour is skipped. A window scheduled inside the skipped hour shifts forward by the same amount: where 2:00 AM becomes 3:00 AM, a 2:30 AM window starts at 3:30 AM.
- When clocks fall **back**, an hour repeats. A window scheduled inside the repeated hour runs at the **first** of its two possible start times.
