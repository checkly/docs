...
```bash Terminal
npx checkly trigger --record --detach
```
...
When you run with `--record`, pressing <kbd>Ctrl</kbd>+<kbd>C</kbd> opens a prompt to cancel the recorded test session. Cancellation applies only to Playwright Check Suite runs inside the session; other check types finish normally.
