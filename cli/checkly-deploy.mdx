...
| `--skip-plan` | - | Deploy without asking Checkly for a plan first. Nothing is previewed and no plan token is used; a deploy that deletes resources still lists them before you confirm. |
...
When the plan shows a check or check group that was edited in the Checkly web app or through the API, and the edit is on a property the CLI can write into your code, the prompt gets a third choice next to applying and cancelling:
...
Choosing it writes your account's current values into the construct files and ends the run without deploying, so you can review the result with `git diff` and deploy again. Only what can be written without guessing is written, so a reason found once the file is read (listed below) can still leave nothing to write:

- On every check: `name`, `description`, `activated`, `muted`, `shouldFail`, `tags`, `locations`, and a `frequency` in whole minutes. Also `degradedResponseTime` and `maxResponseTime` on API checks and URL, TCP, DNS, gRPC, SSL and traceroute monitors; the packet-loss thresholds on ICMP monitors; `period` and `grace` on heartbeat monitors, each written together with its unit; `environmentVariables` on API, browser, multistep and Playwright checks; and the `request` of an API check (`url`, `method`, `headers`, `queryParameters`, `body`, `bodyType`, `basicAuth`, `followRedirects`, `skipSSL`, `ipFamily`) or URL monitor (`url`, `followRedirects`, `skipSSL`, `ipFamily`). Agentic checks take neither `shouldFail` nor `frequency`.
- On a check group: `name`, `activated`, `muted`, `tags`, `locations`, `concurrency`, `environmentVariables`, and `apiCheckDefaults` (`url`, `headers`, `queryParameters`, `basicAuth`).
- The construct has to be declared as `new ApiCheck('logical-id', { … })` in a JavaScript or TypeScript file, with the class imported by name from `checkly` or `checkly/constructs` or destructured from a top-level `require` of it (a namespace import or a re-export from your own module is not recognised) and its options written out as an object literal. An existing value is replaced only when it is itself a literal (a string, number or boolean, or an array or object of those); a property the code does not set is added after the last one of the object that holds it (`request` itself has to exist for `request.body` to be added), in the file's own quoting and indentation. Nothing else in the file is touched.

A change that was also made in your code since the last deploy is written over it, and the output marks it `(replacing a local edit)`; a list such as `tags` that your code changed too is left alone instead, since the CLI cannot merge the two. Every change that cannot be written is listed under `Not updated (edit these by hand)` with its reason: a value written with a helper such as `Frequency.EVERY_5M` or `RetryStrategyBuilder`, `assertions` (write them with `AssertionBuilder`), a reference to another resource (alert channels, private locations, a group), a secret or a locked variable that Checkly does not return, a script or code bundle, a sub-minute frequency, a list your code also changed, options built from a variable or a spread, or a construct the CLI cannot locate in the file (its id is computed, or two constructs share it). TypeScript and JSX files need `typescript` installed in the project, as TypeScript check files already do.

The CLI prints each property it updated with its old and new value. There is no flag for the choice, and the `confirmation_required` envelope does not change.

<Note>
When the plan cannot be computed, the CLI warns `Could not check what this deploy would change` and deploys without one; `--skip-plan` takes the same path on purpose. Without a plan nothing is previewed. A deploy that deletes removed resources finds them with a dry run, for which the code bundle is uploaded before the confirmation, and the confirmation lists those deletions; with `--preserve-resources` nothing is listed and the upload happens after you confirm. `--plan-token` and `--prune-relations` are unavailable on that path.
</Note>
...
<ResponseField name="--skip-plan" type="boolean">

Deploy without asking Checkly for a plan first. Nothing is previewed, no plan token is used, and the deploy applies whatever your account looks like when it runs. What the confirmation shows instead is described in the note under [How a deploy previews its changes](#how-a-deploy-previews-its-changes); `--force` skips the confirmation as usual, and `--output` prints the overview and totals only, since there is no plan to diff against. `--skip-preview` is accepted as an alias.

Cannot be combined with `--preview`, `--dry-run`, `--plan-token` or `--prune-relations`, which all need a plan.

**Usage:**

```bash Terminal
npx checkly deploy --skip-plan
npx checkly deploy --skip-plan --force
```
...
A non-forced `checkly deploy` shows you the plan, with the same overview and diffs as [`--preview`](#command-options) but without the `Deploy exactly this plan` line since this run already pins the token, and asks you to apply it:
...
This confirmation is skipped when you pass `--force` (for CI/CD). In agent or CI environments the CLI instead returns a `confirmation_required` JSON envelope carrying the plan and its token, and exits with code `2` rather than prompting; the envelope names the command that deploys that exact plan. Its `changes` list deletions and detachments in full and created and updated resources up to 20, with the rest counted; the `--dry-run` envelope is capped the same way, while the `preview.diff` both carry is not. Without a plan, the prompt lists deletions instead of the preview, as described in the note under [How a deploy previews its changes](#how-a-deploy-previews-its-changes).
