...
- `jsonAnswer()`: The DNS response parsed as JSON. This allows you to target specific fields using JSON path assertions. The response structure varies by record type. [Learn more about using JSON path](/docs/api-checks/assertions/#json-responses-with-json-path).
