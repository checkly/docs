...
To define `assertions` for the `request` of an `DnsMonitor` you should use the `DnsAssertionBuilder`. The `DnsAssertionBuilder` supports the following sources:
...
- `jsonAnswer()`: The DNS response parsed as JSON. This allows you to target specific fields using JSON path assertions. The response structure varies by record type. See JSON response schemas below for all supported record types
...
// renders to a JSON string
"{ source: 'RESPONSE_CODE', regex: '', property: '', comparison: 'EQUALS', target: 'NOERROR' }"
````

- Assert against specific JSON fields in the response

```ts
DnsAssertionBuilder.jsonAnswer('$.Answer.legth').equals(4)
// renders to a JSON string
"{ source: 'JSON_ANSWER', regex: '', property: '$.Answer.length', comparison: 'EQUALS', target: '4' }"
```
