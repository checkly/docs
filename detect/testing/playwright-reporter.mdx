...
| **Auto-detected env vars** | Values of env vars matching common patterns (`SECRET`, `KEY`, `TOKEN`, `PASSWORD`, `CREDENTIAL`, `AUTH`, `PRIVATE`, `API`) | Enabled by default (`autoDetect: true`) |
  ...
  reporter: [
    ...
    createChecklyReporter({
      scrubbing: {
        ...
        // Redact sensitive HTTP headers (pass an array to customize the list)
...
| `autoDetect` | `boolean` | `true` | Auto-detect secrets from env vars matching common patterns |
| `prefix` | `string \| false` | `'CHECKLY_SECRET_'` | Scrub env vars with this prefix; set `false` to disable |
| `sensitiveHeaders` | `boolean \| string[]` | `true` | Redact sensitive HTTP headers; pass an array for a custom list |
