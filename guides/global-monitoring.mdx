...
**Where are your users?** Take the top regions from your analytics and monitor from one location in each. One per region that matters to revenue, not one per country. For a shop that is 60% US, 25% EU, and 10% India, that is `us-east-1`, `eu-central-1`, and `ap-south-1`.

**Where does it run?** Add one location in every cloud region you deploy to, so a bad regional deploy shows up on its own row. Active in Oregon with failover in Ireland means `us-west-2` and `eu-west-1`.

**One control, far from both.** Pick a location with no users and no infrastructure, such as `ap-southeast-2`. If only the control fails, the problem is the path, not the service. It also gives you the worst-case latency you should design for.
