---
title: 'Get Started with Checkly'
description: 'Get started with Checkly by creating your first browser check'
sidebarTitle: 'Quickstart'
---

import CliProjectInit from '/snippets/cli-project-init.mdx';
import CliAuth from '/snippets/cli-auth.mdx';

This guide will walk you through creating your first browser check with Checkly. Choose your preferred approach:

<Tabs>
<Tab title="Monitoring as Code">

Get started with Checkly using the CLI and code-first approach. You'll learn how to set up monitoring for a web application, configure alerts, and deploy your checks to run continuously.

<Accordion title="Prerequisites">
- Node.js 16+ installed
- A Checkly account (sign up at [checklyhq.com](https://checklyhq.com))
</Accordion>

<Steps>
<Step title="Install and Setup">
Create a new Checkly project using our CLI:

<CliProjectInit/>

```bash Terminal
cd my-checkly-project
```

This creates a project with example checks and all necessary configuration files.

Login to your Checkly account:

<CliAuth/>
</Step>

<Step title="Create Your First Browser Check">
Let's create a browser check that monitors a login flow. Create a new file `src/login-check.spec.ts`:

```typescript
import { test, expect } from '@playwright/test'

test('User can login successfully', async ({ page }) => {
  // Navigate to login page
  await page.goto('https://app.checklyhq.com/login')
  
  // Fill in login credentials
  await page.getByLabel('Email address').fill('john@example.com')
  await page.getByLabel('Password').fill('mypassword')
  
  // Submit the form
  await page.getByRole('button', { name: 'Log In' }).click()
  
  // Verify successful login
  await expect(page).toHaveURL(/.*dashboard/)
  await expect(page.getByText('Welcome back')).toBeVisible()
  
  // Take a screenshot for verification
  await page.screenshot({ path: 'dashboard.png', fullPage: true })
})
```
</Step>

<Step title="Configure Your Check">
Update the check configuration in `checkly.config.ts`:

```typescript
import { ChecklyConfig } from 'checkly'

const config: ChecklyConfig = {
  projectName: 'My Web App Monitoring',
  logicalId: 'my-web-app',
  repoUrl: 'https://github.com/your-org/your-repo',
  checks: {
    locations: ['us-east-1', 'eu-west-1'],
    tags: ['login', 'critical'],
    runtimeId: '2024.02',
    browserChecks: {
      frequency: 10, // Run every 10 minutes
      testMatch: '**/login-check.spec.ts'
    }
  },
  cli: {
    runLocation: 'eu-west-1'
  }
}

export default config
```
</Step>

<Step title="Add Environment Variables">
For sensitive data like credentials, use environment variables. Create a `.env` file:

```bash
LOGIN_EMAIL=john@example.com
LOGIN_PASSWORD=mypassword
```

Update your test to use these variables:

```typescript
test('User can login successfully', async ({ page }) => {
  await page.goto('https://app.checklyhq.com/login')
  
  await page.getByLabel('Email address').fill(process.env.LOGIN_EMAIL!)
  await page.getByLabel('Password').fill(process.env.LOGIN_PASSWORD!)
  
  await page.getByRole('button', { name: 'Log In' }).click()
  await expect(page).toHaveURL(/.*dashboard/)
})
```
</Step>

<Step title="Configure Alerts">
Add alert channels to get notified when checks fail. Update your `checkly.config.ts`:

```typescript
import { EmailAlertChannel, SlackAlertChannel } from 'checkly'

// Create alert channels
const emailChannel = new EmailAlertChannel('email-alerts', {
  address: 'alerts@yourcompany.com'
})

const slackChannel = new SlackAlertChannel('slack-alerts', {
  url: process.env.SLACK_WEBHOOK_URL!,
  channel: '#monitoring'
})

const config: ChecklyConfig = {
  // ... previous config
  alertChannels: [emailChannel, slackChannel],
  checks: {
    // ... previous config
    alertChannels: [emailChannel, slackChannel]
  }
}
```
</Step>

<Step title="Test Locally">
Before deploying, test your check locally:

```bash Terminal
npx checkly test
```

This runs your browser check using Playwright and shows you exactly what will run in production.
</Step>

<Step title="Deploy Your Checks">
Deploy your checks to Checkly's global infrastructure:

```bash Terminal
npx checkly deploy
```

Your checks will now run automatically according to your schedule and alert you if anything fails.
</Step>

<Step title="Monitor and Iterate">
Visit your Checkly dashboard to:

- View check results and performance metrics
- Analyze screenshots and traces from failed checks
- Configure additional alert channels
- Set up status pages for your users
</Step>
</Steps>

## Next Steps

Now that you have your first browser check running, explore these advanced features:

- **API Checks**: Monitor your REST APIs and GraphQL endpoints
- **Visual Regression Testing**: Catch UI changes automatically
- **Private Locations**: Run checks from your own infrastructure
- **CI/CD Integration**: Integrate checks into your deployment pipeline
- **Rocky AI**: Get AI-powered insights into check failures

## Best Practices

- **Start Simple**: Begin with basic page load and navigation tests
- **Use Page Object Model**: Structure complex tests for maintainability
- **Monitor Critical Paths**: Focus on user journeys that impact revenue
- **Set Appropriate Frequencies**: Balance coverage with resource usage
- **Use Environment Variables**: Keep sensitive data secure
- **Tag Your Checks**: Organize monitoring by feature or team

Congratulations! You've successfully set up automated browser monitoring with Checkly. Your application is now being continuously monitored from multiple global locations.

</Tab>
<Tab title="Starting in the UI">

Get started with Checkly using the web interface. This approach is perfect for getting up and running quickly without any local setup.

<Accordion title="Prerequisites">
- A Checkly account (sign up at [checklyhq.com](https://checklyhq.com))
- The URL of a website you want to monitor
</Accordion>

<Steps>
<Step title="Create Your First Check">
1. **Log in** to your Checkly dashboard at [app.checklyhq.com](https://app.checklyhq.com)
2. **Click "Create Check"** from the main dashboard
3. **Choose "Browser Check"** for comprehensive website monitoring
4. **Enter your website URL** (e.g., `https://your-website.com`)
5. **Give your check a name** like "Homepage Monitor"
</Step>

<Step title="Configure Basic Settings">
Configure your check settings:

- **Locations**: Select 2-3 global locations (e.g., US East, EU West, Asia Pacific)
- **Frequency**: Start with every 5 minutes for critical pages
- **Tags**: Add tags like `production` or `homepage` for organization

<Note>
For your first check, the default settings work well. You can always adjust them later.
</Note>
</Step>

<Step title="Set Up Alerts">
Configure who gets notified when issues are detected:

1. **Click "Alert Settings"** in the check configuration
2. **Add your email address** for notifications
3. **Choose alert conditions**: 
   - Alert after 1 failure (for critical pages)
   - Alert after 2 failures (for less critical pages)
4. **Set recovery notifications** to know when issues are resolved
</Step>

<Step title="Test and Save">
Before activating your check:

1. **Click "Test Check"** to run it once and verify it works
2. **Review the results** to ensure your website loads correctly
3. **Check for any errors** in the console or network tabs
4. **Click "Save and Activate"** to start continuous monitoring
</Step>

<Step title="View Results">
Once your check is active:

1. **Visit your dashboard** to see real-time results
2. **Check the results page** for detailed performance metrics
3. **Review screenshots** and timing information
4. **Set up additional checks** for other critical pages
</Step>
</Steps>

## Next Steps

Now that you have your first check running:

- **Add API Checks**: Monitor your backend services and APIs
- **Create Check Groups**: Organize related checks together
- **Set up Status Pages**: Share uptime information with users
- **Explore Integrations**: Connect with Slack, PagerDuty, or other tools

## Upgrading to Monitoring as Code

As your monitoring needs grow, consider upgrading to our CLI approach:

- **Version Control**: Keep your monitoring configuration in Git
- **Team Collaboration**: Review changes through pull requests  
- **CI/CD Integration**: Deploy monitoring alongside your applications
- **Advanced Scripting**: Write custom Playwright tests for complex flows

Ready to make the switch? Check out the **Monitoring as Code** tab above!

</Tab>
</Tabs>