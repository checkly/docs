...
<Tip>Learn more about writing setup and teardown scripts in [the setup and teardown scripts documentation](/detect/synthetic-monitoring/api-checks/set-up-and-tear-down).</Tip>
...
- `jsonBody(property?)`: Assert the JSON response body. The property argument accepts a [JSON path expression](/detect/synthetic-monitoring/api-checks/assertions#json-responses-with-json-path)
...
Learn more in our docs on [Assertions](/detect/synthetic-monitoring/api-checks/assertions).
