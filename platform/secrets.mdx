...
canonical: 'https://www.checklyhq.com/docs/platform/secrets/'
---
...
<Tip>
  If you manage many credentials with Monitoring as Code, learn how to [load them from an external secrets manager and deploy them at group or check scope](/platform/manage-secrets-at-scale).
</Tip>
...
new ApiCheck('authenticated-api-check', {
  ...
  request: {
    ...
    headers: [
      ...
      { key: 'Authorization', value: 'Bearer {{API_SECRET_KEY}}' },
...
new ApiCheck('database-health-check', {
  ...
  request: {
    ...
    headers: [
      { key: 'Authorization', value: 'Bearer {{DB_MONITORING_TOKEN}}' }
    ],
    body: JSON.stringify({
