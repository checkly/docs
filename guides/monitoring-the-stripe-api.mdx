...
tags:
canonical: 'https://www.checklyhq.com/docs/guides/monitoring-the-stripe-api/'
---
...
Checkly specialises in API monitoring and allows users to run API checks on a schedule from [global locations](/concepts/locations). We can combine these checks with [custom alerting](/communicate/alerts/overview) to be able to quickly respond and remediate potential API issues.
...
1. Importing existing [Swagger/OpenAPI specs](/detect/synthetic-monitoring/api-checks/configuration#import-a-swagger--openapi-specification) or even [cURL commands](/detect/synthetic-monitoring/api-checks/configuration#import-a-curl-request) using built-in functionality.
2. [Defining our API checks as code](/guides/startup-guide-detect-communicate-resolve) to scale our setup while lowering maintenance needs.
...
<Card title="Monitoring as Code" href="/guides/startup-guide-detect-communicate-resolve">
